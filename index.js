...
async function start(client = new Client()) {
    ...
    console.log(color(figlet.textSync('  RengeBot', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
        ...
        client.getAllGroups().then((groups) => {
            ...
            if (groups.length > groupLimit) {
                ...
                client.simulateTyping(chat.groupMetadata.id, true).then(async () => {
                    client.sendText(chat.groupMetadata.id, `Hai guys 👋 perkenalkan saya RengeBot. Untuk melihat perintah/menu yang tersedia pada bot, kirim ${prefix}menu`)
                })
            }
        })
    })
